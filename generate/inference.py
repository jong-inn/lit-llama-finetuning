...
def main(
    save_path: Path = Path("."),
    data_path: Path = Path("data/test/camel-ai_ai_society_1000.json"),
    ...
    Args:
        ...
        temperature: A value controlling the randomness of the sampling process. Higher values result in more random
            samples.
    """
    assert save_path.as_posix().endswith('.json')
    ...
    with open(save_path, 'w') as f:
        for i, d in enumerate(data):
            if i >= 500:
                break
            sample = {"instruction": d["instruction"], "input": d["input"]}
            ...
            print(f"\n\nprogress: {round(i / len(data) * 100, 2)} %")
